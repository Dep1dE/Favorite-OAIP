...
public:
    ...
    void erase(size_t pos, size_t k) {
        if (empty()) {
            throw std::out_of_range("vector is clear");
        }
        else if (size < (pos + k)) {
            throw std::out_of_range("idi nahuy");
        }
        else {
...
int main() {
    Vector<int>p, b;  
    string s, subS="",finalS;
    cout << "¬ведите строку чисел через зап€тую"<<'\n';
    cin >> s;
    ...
    if(!b.empty()){
        ...
        finalS += to_string(sum / b.Size());
    }
    if (finalS[finalS.size() - 1] == ',') {
        finalS.pop_back();
    }
    cout << finalS << '\n';
